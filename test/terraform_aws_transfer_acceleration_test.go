...
func TestTerraformTransferAccelerationExample(t *testing.T) {
	...
	testName := fmt.Sprintf("terratest-s3-bucket-transfer-acceleration-%s", strings.ToLower(random.UniqueId()))

	tags := map[string]interface{}{
		"Automation": "Terraform",
		"Terratest":  "yes",
		"Test":       "TestTerraformTransferAccelerationExample",
	}

	terraformOptions := terraform.WithDefaultRetryableErrors(t, &terraform.Options{
		// TerraformDir is where the terraform state is found.
		TerraformDir: "../examples/transfer_acceleration",
