...
func TestTerraformEncryptedBucketKeyExample(t *testing.T) {
	...
	testName := fmt.Sprintf("terratest-s3-bucket-encrypted-bucket-key-%s", strings.ToLower(random.UniqueId()))

	tags := map[string]interface{}{
		"Automation": "Terraform",
		"Terratest":  "yes",
		"Test":       "TestTerraformEncryptedBucketKeyExample",
	}

	terraformOptions := terraform.WithDefaultRetryableErrors(t, &terraform.Options{
		// TerraformDir is where the terraform state is found.
		TerraformDir: "../examples/encrypted_bucket_key",
