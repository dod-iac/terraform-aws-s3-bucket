...
| <a name="input_lifecycle_rules"></a> [lifecycle\_rules](#input\_lifecycle\_rules) | A list of lifecycle rules. | <pre>list(object({<br>    id      = string<br>    enabled = bool<br>    prefix  = optional(string)<br>    tags    = optional(map(string))<br>    transitions = list(object({<br>      date          = optional(string)<br>      days          = optional(number)<br>      storage_class = string<br>    }))<br>  }))</pre> | `[]` | no |
