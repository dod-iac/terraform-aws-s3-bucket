...
| <a name="input_lifecycle_rules"></a> [lifecycle\_rules](#input\_lifecycle\_rules) | A list of lifecyle rules. | <pre>object({<br>    id      = string<br>    enabled = bool<br>    prefix  = string<br>    tags    = map(string)<br>    transitions = list(object({<br>      date          = string<br>      days          = number<br>      storage_class = string<br>    }))<br>  })</pre> | `[]` | no |
